...
def _try_load_bundle(art_dir: str):
    ...
    - dict con claves: pipeline/pipe/model, feature_cols/X_cols/cols, label_names/classes, thresholds/thr_map
    - tupla/lista: (pipeline, feature_cols?, label_names?, thresholds?)
    - pipeline "pelado"
    Devuelve dict con: {"pipe", "cols", "labels", "thr_map"} o None si no existe.
    """
    import joblib
    ...
    # Caso 2: tupla/lista (pipeline[, cols[, labels[, thr_map]]])
    ...
    # Caso 3: pipeline “pelado”
    else:
        pipe = obj
        # intentar leer classes_ si existe
...
def load_artifacts(artifacts_dir="artifacts"):
    """
    Prioridad:
    1) bundle (si tiene pipeline válido)
    2) trío estándar: pipeline_ovr_logreg.joblib + feature_cols.csv + label_names.csv (+ thresholds.json opcional)
    """
    import joblib
    b = _try_load_bundle(artifacts_dir)
    if b is not None and b.get("pipe") is not None:
        # si el bundle no trae thresholds, intenta thresholds.json
...
def predict_labels(feats: dict, artifacts_dir="artifacts", threshold=0.5):
    ...
    if "cols_csv" in loaded:
        ...
        if labels is None:
            # sin labels no podemos mapear predict_proba
            return None, None

    # Construir fila ordenada por cols
    ...
    if hasattr(pipe, "predict_proba"):
        ...
        if hasattr(pipe, "decision_function"):
            ...
            import numpy as _np
            y = 1/(1+_np.exp(-_np.asarray(df)[0]))
        else:
            # sin proba/decision no podemos umbralizar
            return None, None
